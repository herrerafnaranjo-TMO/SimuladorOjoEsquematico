...
# =================================================================
# 1. CONFIGURACIÓN Y BLINDAJE VISUAL
# =================================================================
...
# =================================================================
# 4. RENDERIZADO 3D (TU CÓDIGO PREMIUM RESTAURADO)
# =================================================================
def generar_ojo_3d(res):
    ...
    # M1 (Cian)
    xm1 = r_c - r_c * np.cos(t_arco)
    ym1, zm1 = r_c * np.sin(t_arco) * np.sin(ang), r_c * np.sin(t_arco) * np.cos(ang)
    ...
    # M2 (Magenta)
    ang2 = ang + np.pi/2
    xm2 = r_c - r_c * np.cos(t_arco)
    ym2, zm2 = r_c * np.sin(t_arco) * np.sin(ang2), r_c * np.sin(t_arco) * np.cos(ang2)
    ...
    # DISEÑO DE ESCENA (LEGIBILIDAD TOTAL)
    fig.update_layout(
        scene=dict(
            ...
            camera=dict(eye=dict(x=1.5, y=-2.5, z=0.8))
        ),
        paper_bgcolor="#F8F9FA", font=dict(color="#1A202C", size=12), margin=dict(l=0, r=0, b=0, t=0), showlegend=True
    )
    return fig
